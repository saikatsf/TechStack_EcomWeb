...
@section('content')
    <div class="main container p-2 my-2 border">
        ...
        <hr>
        <form class="form-inline m-1 w-25" method="POST" action="forgotPassOTP">
            @csrf
            <input type="text" class="form-control" name="forgotPemail" value="{{session('forgotPassEmail')}}" hidden>
            <div class="form-group m-1">
            <label for="forgotPotp">Enter OTP :</label>
            <input type="text" class="form-control" name="forgotPotp" id="forgotPotp">
            @error('forgotPotp')
